...
const niiloPersona = `
Olet Niilo, Novyra Technologiesin tekoälyassistentti. Puhu rennosti mutta ammattitaitoisesti.
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (!CHAT_ENABLED) {
      return res
        .status(503)
        .json({ error: "OPENAI_API_KEY puuttuu palvelimelta." });
    }
...
app.post("/api/voice-call", async (req, res) => {
  try {
    ...
    const reply =
      ...
      "En valitettavasti kuullut kysymystäsi kunnolla.";
...
app.post("/api/log", async (req, res) => {
  try {
    ...
    const crmUrl =
      process.env.CRM_API_URL || "https://novyracrm.com/api/logs";
